  ...
  return (
    <div className="fixed inset-0 z-[-1] overflow-hidden bg-[#030305]">
      ...
      <motion.div 
        ...
        className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] rounded-full bg-neon-purple/20 blur-[120px] mix-blend-screen" 
      />
      <motion.div 
        animate={{ x: ["0%", "-10%", "0%"], y: ["0%", "-5%", "0%"], scale: [1, 1.2, 1] }} 
        transition={{ duration: 20, repeat: Infinity, ease: "easeInOut", delay: 2 }}
        className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] rounded-full bg-neon-cyan/20 blur-[150px] mix-blend-screen" 
      />
      <motion.div 
        animate={{ x: ["0%", "15%", "0%"], y: ["0%", "-15%", "0%"], scale: [1, 1.1, 1] }} 
        transition={{ duration: 18, repeat: Infinity, ease: "easeInOut", delay: 4 }}
        className="absolute top-[20%] right-[20%] w-[40%] h-[50%] rounded-full bg-neon-pink/15 blur-[100px] mix-blend-screen" 
      />
      <motion.div 
        animate={{ x: ["0%", "-20%", "0%"], y: ["0%", "20%", "0%"], scale: [1, 1.3, 1] }} 
        transition={{ duration: 25, repeat: Infinity, ease: "easeInOut", delay: 1 }}
        className="absolute bottom-[20%] left-[20%] w-[40%] h-[40%] rounded-full bg-blue-500/10 blur-[120px] mix-blend-screen" 
      />
      <motion.div 
        animate={{ x: ["0%", "10%", "0%"], y: ["0%", "10%", "0%"], scale: [1, 1.2, 1] }} 
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut", delay: 3 }}
        className="absolute top-[40%] left-[40%] w-[30%] h-[30%] rounded-full bg-purple-500/10 blur-[90px] mix-blend-screen" 
      />
      ...
      {particles.map((p) => (
        <motion.div
          key={p.id}
          className={`absolute rounded-full ${p.color} shadow-[0_0_15px_rgba(255,255,255,0.4)]`}
          style={{
            width: p.size,
            height: p.size,
            left: `${p.x}%`,
      ...
      {lines.map((l) => (
        <motion.div
          key={`line-${l.id}`}
          className="absolute top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-neon-cyan/40 to-transparent"
          style={{ left: `${l.x}%` }}
          animate={{
            opacity: [0, 1, 0],
            y: ["-100%", "100%"],
